...
void drawParameters(void)
{
  if (toggleParams) {
    ...
    glColor3fv(white);
    /*  Display parameters */
    printAt(5,height,"Angle=%d,%d  Dim=%.1f FOV=%d Light=%s Alpha=%.2f",
	    th,ph,dim,fov,alpha,toggleLight?"On":"Off");
    if (toggleLight) {
    ...
    printAt(5,height +=20,"Blend Func=%s",aone?"GL_ONE":"GL_ONE_MINUS_SRC_ALPHA");
    if (toggleOverlay) {
